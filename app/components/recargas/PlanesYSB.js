...
const sim = ProductsCopies.find(product => product.id === "paquetegb");
const planes = ProductsCopies.find(product => product.id === "paquetegb").variant;

const PlanesYSB = ({ captions }) => {
        ...
        <section className="sm:px-0 md:px-0 lg:px-0 px-4 bg-[#F2F2F2]">
            ...
            <div className="max-w-7xl mx-auto">
                ...
                >
                    ...
                    {planes.map((item, idx) => (
                        <SwiperSlide key={idx}>
                            <div onClick={()=>handleClick(idx)} className={`${item.destacado ? "bg-[#FF5F00]" : "bg-initial"} ${isActive === idx ? "shadow-2xl bg-[#E3D3BD]" : "bg-[#B85564]"} hover:shadow-2xl transition-all cursor-pointer rounded-[20px] p-1 relative`}>
                                {
                                    item.destacado ?
                                ...
                                <h3 className={`text-center font-bold ${(item.destacado || isActive=== idx) ? "text-[#B85564]" : "text-white"} text-[14px] sm:text-[22px] md:text-[22px] lg:text-[22px] py-4`}>{sim.title}</h3>
                                </div>
